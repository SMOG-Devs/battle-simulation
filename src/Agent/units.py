...
class Infantry(Unit):
    ...
    def attack(self, enemy_regiment):
        def inside_of_grid(troop: Infantry):
            return 0 < self.battle_front.positions[troop][0] < self.battle_front.shape[0] and \
                   0 < self.battle_front.positions[troop][1] < self.battle_front.shape[0]

        if not inside_of_grid(self):
            return

        for neighbor in self.battle_front.neighbors(self, distance=self.range).to_list():
    ...
    def move(self, enemy_position: Tuple[int, int], regiment_position: Tuple[int, int]):
        match self.regiment_order:
            case Orders.MoveAndAttack:
                self.__calculateVector(enemy_position, regiment_position)
                destination = self.pos[0] + self.vector[0], self.pos[1] + self.vector[1]
                if destination in self.battle_front.empty:
                    self.battle_front.move_by(self, self.vector)
                self.pos = self.battle_front.positions[self]
